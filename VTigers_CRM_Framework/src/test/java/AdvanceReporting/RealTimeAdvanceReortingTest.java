...
public class RealTimeAdvanceReortingTest extends BaseClass {

	@Test(groups = { "Smoke Test", "Regression Test" })
	public void createOrgTest() throws Throwable {
		ListenerImplemetation.test.log(Status.INFO, "Navigate to VTigers application"); // with static variable

		homePage hp = new homePage(driver);
		ListenerImplemetation.test.log(Status.INFO, "Click on organizations");
		hp.getOrganizationsLink().click();

		ListenerImplemetation.test.log(Status.INFO, "Click on Create organizations");
		...
		if (headerInfo.contains(OrgName)) {
			ListenerImplemetation.test.log(Status.PASS, OrgName + " : is available in Header");
		} else {
			ListenerImplemetation.test.log(Status.FAIL, OrgName + " : is available in Header");
		}
		// verify org table contains org name
		if (actualOrgName.equals(OrgName+2)) {
			ListenerImplemetation.test.log(Status.PASS, OrgName + " : is available in org table");
		} else {
			ListenerImplemetation.test.log(Status.FAIL, OrgName + " : is not available in org table");
	...
	public void createOrgWithInsdustryAndTypeTest() throws Throwable {
		...
		ListenerImplemetation.test.log(Status.INFO, "Click on Organizations link");
		homePage hp = new homePage(driver);
		hp.getOrganizationsLink().click();

		ListenerImplemetation.test.log(Status.INFO, "Click on create Organizations link");
		organizationsPage orgPage = new organizationsPage(driver);
		orgPage.getCrateOrgLink().click();

		ListenerImplemetation.test.log(Status.INFO, "Enter Details");
		int randomInt = jUtil.getRandomInteger();
		String OrgName = eUtil.getDataFromExcel("orgData", 1, 2) + randomInt;
		...
		ListenerImplemetation.test.log(Status.INFO, "Save Details");
		createOrgPage.getSaveButton().click();
		...
		if (headerInfo.contains(OrgName)) {
			...
			ListenerImplemetation.test.log(Status.FAIL,OrgName + ": is not available in Header");
		}
		// verify org table contains org name
		...
		if (actualIndustry.equals(OrgIndustry)) {
			ListenerImplemetation.test.log(Status.PASS,OrgIndustry + ": is available in Header");
		} else {
			ListenerImplemetation.test.log(Status.FAIL,OrgIndustry + ": is not available in Header");
		}
		// verify org table contains org name
		if (actualType.equals(OrgType)) {
			ListenerImplemetation.test.log(Status.PASS,OrgType + ": is available in org table");
		} else {
			ListenerImplemetation.test.log(Status.FAIL,OrgType + ": is not available in org table");

		}
	}
	@Test(groups = "Regression Test")
	public void CreateOrgWithPhoneNumberTest() throws Throwable {
		ListenerImplemetation.test.log(Status.INFO, "Navigate to VTigers application");
		...
		ListenerImplemetation.test.log(Status.INFO, "Enter deatails");
		int randomInt = jUtil.getRandomInteger();
		...
		if (actualPhone.equals(OrgPhone)) {
			ListenerImplemetation.test.log(Status.PASS,OrgPhone + ": is available in org table ");
		} else {
			ListenerImplemetation.test.log(Status.FAIL,OrgPhone + ": is not available in org table ");
		}
	}
